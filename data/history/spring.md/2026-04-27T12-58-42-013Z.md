...
![library vs framework](assets/spring/framework_vs_library.png)
...
class SomeController {
  ...
  @GetMapping()
  List<Dto> getThem(@RequestParam int page, @RequestParam int size) {
    return repository.findThem(Pageable.ofSize(size).withPage(page))
          .stream()
          .map(Dto::new)
          .toList();
  }
...
class SomeController {
  ...
  @GetMapping()
  List<Dto> getThemSorted(
      @PageableDefault(size = 10, sort = "name", page = 0) 
      Pageable pageable
  ) {
    return repository.findThem(Sort.by("name"))...
  }
}
```
<!-- .element: class="stretch fragment"-->
...
```
Sort.by("someVariable").descending()
Sort.by("someVariable").descending().and(Sort.by("name"))
...
```
<!-- .element: class="stretch fragsment"-->